...
sudo /opt/smax/2020.11/install --nfs-server "$NFS_SERVER"  --nfs-folder "$NFS_PATH_CORE"  --registry-url "gcr.io"  --registry-username "oauth2accesstoken"  --registry-orgname "$REGISTRY_ORG"  --registry-password "$gcrpwd"  --external-access-host "$EXT_ACCESS_FQDN"  --cloud-provider gcp --loadbalancer-info "LOADBALANCERIP=$LB_EXT_IP" --db-url "jdbc:postgresql://$PSQL_DB_HOST:5432/cdfapiserverdb" --db-user "cdfapiserver" --db-password "0f5546d03a520e627f17719865aa53cd"
